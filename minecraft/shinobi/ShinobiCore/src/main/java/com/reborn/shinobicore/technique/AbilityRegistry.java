...
public final class AbilityRegistry implements com.reborn.shinobicore.api.TechniqueRegistry {
    ...
    private void parseOne(String id, Map<?, ?> m) {
        ...
        // Access prerequisites (requires:). Each entry is a small map; tolerant
        // parse — a malformed entry is logged and skipped, never fatal.
        List<Requirement> requires = new ArrayList<>();
        Object rawReq = m.get("requires");
        if (rawReq instanceof List<?> reqList) {
            for (Object o : reqList) {
                if (!(o instanceof Map<?, ?> rm)) continue;
                var parsed = Requirement.parse(rm);
                if (parsed.isPresent()) requires.add(parsed.get());
                else log.warning("Ability " + id + ": entrée 'requires' illisible, ignorée.");
            }
        }
        ...
        byId.put(id, new Ability(id, name, category, rank, execution,
                minigame, difficulty, mudras, steps, description, meta, requires));
    }
