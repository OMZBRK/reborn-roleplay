...
const NAV: NavItem[] = [
  ...
  { href: '/whitelist', label: 'Whitelist', icon: '◈' },
...
export default function PanelLayout({
  ...
  if (!ready) {
    // Pendant le check d'auth on rend rien — evite le flash de UI staff
    // pour un visiteur non-authentifie.
    return null;
  }
    ...
    <div className="min-h-screen flex">
      <aside className="w-60 shrink-0 border-r border-[var(--color-border)] bg-[var(--color-surface)] flex flex-col">
        ...
        <button
          ...
          className="mx-3 mb-4 rounded-[10px] border border-[var(--color-border-strong)] py-2 text-sm text-[var(--color-foreground-subtle)] hover:bg-[var(--color-danger-soft)] hover:text-[var(--color-danger)] hover:border-[var(--color-danger)] transition-colors"
        >
          Deconnexion
        </button>
      </aside>
